...
export interface ProjectDocument {
  id: string
  name: string
  type: "doc" | "sheet" | "slide" | "brief" | "link" | "upload"
  url?: string
  content?: string
  fileName?: string
  fileSize?: string
...
export type AgentToolStatus = "active" | "not_linked" | "error"
export type AgentToolCategory = "system" | "developer" | "communication" | "creative" | "marketing" | "productivity"
...
// Per-agent tool sets
export const agentToolSets: Record<string, { active: AgentTool[]; available: AgentTool[] }> = {
  dave: {
    active: [
...
export type KanbanStatus = "Brief Needed" | "Working" | "Need Direction" | "Stalled" | "Done"
...
export interface Project {
  ...
  date: string
  progress: number
  status: "Active" | "Pre-Launch" | "Completed" | "On Hold"
  kanbanStatus: KanbanStatus
  deadline: string
  deadlineDate?: string // ISO date e.g. "2026-02-17"
  agents: string[]
  tasks: Task[]
  client?: string
  brief?: string
  teamMembers?: string[] // agent + employee ids
...
export interface ChatMessage {
  ...
  senderName: string
  senderAvatar?: string
  senderType: "admin" | "agent" | "employee"
  message: string
  timestamp: string
  mentions?: string[] // ids of mentioned people
}

export interface ChatChannel {
  id: string
  name: string // displayed with # prefix
  description?: string
}
...
export function getAllMembers(): { id: string; name: string; type: "admin" | "agent" | "employee" }[] {
  return [
