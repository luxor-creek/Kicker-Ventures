...
export function TaskDetailPanel({
  ...
  const assignedAgentObjects = task.assignedAgents
          ...
          <div className="space-y-5 px-6 py-5">
            ...
            <div>
              ...
              {editing ? (
                ...
                <div className="flex items-center gap-2 text-sm text-foreground">
                  ...
                  {task.deadline && (
                    <span className="text-xs text-muted-foreground">(Created {task.date})</span>
                  )}
                </div>
              )}
            </div>
            ...
            <div>
              ...
              {assignedAgentObjects.length > 0 ? (
                  ...
                  {assignedAgentObjects.map((agent) => {
                    ...
                    return (
                      <div key={agent.id}>
                        ...
                        {confirmRemoveAgentId === agent.id && (
                          <p className="px-2.5 py-1 text-[11px] text-destructive">
                ...
                <p className="mb-1 text-xs text-muted-foreground">No agents assigned yet.</p>
              )}
                    ...
                    {unassignedAgents.length > 0 ? (
                      unassignedAgents.map((agent) => (
                        ...
                        >
                          ...
                          {projectAgents.includes(agent.id) && (
                            <Badge variant="outline" className="ml-auto text-[8px]">On Project</Badge>
                          )}
                        </button>
                      ))
                    ) : (
                      <p className="px-2.5 py-1.5 text-xs text-muted-foreground">All agents are already assigned.</p>
                    )}
                  </div>
                  <button
