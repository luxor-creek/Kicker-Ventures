...
/* ----- Tool Logo Component ----- */
function ToolLogo({ src, name, size = "sm" }: { src: string; name: string; size?: "sm" | "md" }) {
  ...
  if (imgError) {
    return (
      <div className={cn(s, "flex items-center justify-center rounded bg-muted", fs, "font-bold text-muted-foreground")}>
        {name[0]}
      </div>
    )
  }
...
/* ----- Status Dot ----- */
function StatusDot({ status }: { status: AgentToolStatus }) {
...
/* ===== TOOL INFO MODAL (for pre-installed active tools) ===== */
function ToolInfoModal({ tool, onClose }: { tool: AgentTool; onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={onClose}>
      <div className="w-full max-w-md rounded-xl border border-border bg-card p-5 shadow-xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between rounded-lg border border-border px-3 py-2">
          ...
          {tool.preInstalled && (
            <span className="text-[10px] text-muted-foreground">Set by admin</span>
          )}
        </div>
      </div>
    </div>
...
/* ===== CONNECT MODAL (for optional/available tools) ===== */
function ConnectModal({
  ...
  tool: AgentTool
  onClose: () => void
  onConnected: (toolId: string) => void
}) {
  ...
  function handleTest() {
    setTesting(true)
    setTestResult("idle")
    // Simulate connection test: 50/50 success or error for demo
    setTimeout(() => {
...
/* ===== AGENT TOOL GRID (Active Tools) ===== */
function ActiveToolGrid({
  tools,
  onClickTool,
}: {
  tools: AgentTool[]
  onClickTool: (tool: AgentTool) => void
}) {
  // Group by category
  ...
  const categoryOrder: AgentToolCategory[] = ["developer", "marketing", "creative", "communication", "productivity", "system"]
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-1.5">
                      ...
                      {tool.category === "system" && (
                        <Badge variant="outline" className="text-[8px] py-0 px-1">System</Badge>
                      )}
                    </div>
                    <p className="truncate text-[10px] text-muted-foreground">{tool.description}</p>
                  </div>
                  {tool.requiresSetup && tool.status === "active" && (
...
function AvailableToolList({
  ...
  const categoryOrder: AgentToolCategory[] = ["communication", "creative", "marketing", "productivity", "developer", "system"]
  ...
  return (
    <div>
      ...
      <div className="space-y-3">
            ...
            <div key={cat}>
              ...
              <div className="space-y-1">
                {items.map((tool) => {
                  ...
                  return (
                    <div key={tool.id} className="flex items-center gap-3 rounded-lg border border-border bg-card px-3 py-2.5">
                      <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-md bg-muted">
                        <ToolLogo src={tool.logo} name={tool.name} />
                      </div>
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-1.5">
                          <p className="truncate text-xs font-semibold text-card-foreground">{tool.name}</p>
                          {tool.requiresSetup && !isConnected && (
                            <Badge variant="outline" className="text-[8px] py-0 px-1 border-amber-300 text-amber-600">Requires Setup</Badge>
                          )}
                        </div>
                        <p className="truncate text-[10px] text-muted-foreground">{tool.description}</p>
                      </div>
                      {isConnected ? (
                        <span className="flex items-center gap-1.5 rounded-md bg-emerald-500/10 px-2.5 py-1 text-[10px] font-semibold text-emerald-600">
                          <Check className="h-3 w-3" />
                          Connected
                        </span>
                      ) : (
        ...
        {filtered.length === 0 && (
          <p className="py-4 text-center text-xs text-muted-foreground">No matching tools found</p>
        )}
      </div>
    </div>
  )
...
const rolePresets = [
  "CTO", "CMO", "Social Media", "Customer Service", "Lead Gen",
  "WordPress Specialist", "Shopify Expert", "SEO Analyst",
...
interface CreateAgentForm {
  ...
  websiteUrl: string
  websiteStatus: "idle" | "testing" | "active" | "error"
...
function CreateAgentModal({ onClose, onCreate }: { onClose: () => void; onCreate: (agent: Agent) => void }) {
  ...
  const [step, setStep] = useState<"config" | "sandbox">("config")
  ...
  function handleCreate() {
    const newAgent: Agent = {
      ...
      status: "idle",
      expertise: form.systemPrompt.identity || undefined,
      profileNotes: form.systemPrompt.tone || undefined,
    }
    onCreate(newAgent)
    onClose()
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/60 p-4 pt-8 sm:pt-12" onClick={onClose}>
          ...
          <div className="flex items-start gap-4">
            ...
            >
              {form.avatarPreview ? (
                ...
                <Upload className="h-5 w-5 text-muted-foreground transition-colors group-hover:text-accent" />
              )}
                  ...
                  <>
                    ...
                    <div className="absolute left-0 top-full z-20 mt-1 max-h-48 w-56 overflow-y-auto rounded-lg border border-border bg-card py-1 shadow-lg">
                      {rolePresets.map((r) => (
                        <button
                          key={r}
                          onClick={() => { setForm((prev) => ({ ...prev, role: r })); setShowRoleDropdown(false) }}
                          className={cn(
          ...
          <div className="grid gap-5 p-5 sm:grid-cols-2 sm:p-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                {form.creativity < 40 && (
                  <p className="mt-1 text-[9px] text-accent">Recommended for specialists and code-heavy roles</p>
                )}
              </div>
                ...
                <div className="space-y-1">
                  {guardrailDefaults.map((g) => (
                    ...
                    >
                      ...
                      {g.id === "human-approval" && form.guardrails[g.id] && (
                        <Badge variant="outline" className="ml-auto text-[8px] border-amber-300 py-0 px-1 text-amber-600">Recommended</Badge>
                      )}
                    </label>
                  ))}
                </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="rounded-lg border border-border">
                ...
                <div className="flex h-64 items-center justify-center bg-muted/20">
                  {form.websiteUrl && form.websiteStatus === "active" ? (
...
export function AgentsView() {
  ...
  const [viewingTool, setViewingTool] = useState<AgentTool | null>(null)
  const [connectingTool, setConnectingTool] = useState<{ tool: AgentTool; agentId: string } | null>(null)
  const [connectedToolsByAgent, setConnectedToolsByAgent] = useState<Record<string, Set<string>>>({})
  const [toolSearches, setToolSearches] = useState<Record<string, string>>({})
  ...
  function saveEditing() {
    ...
    setAgentList((prev) =>
      prev.map((a) =>
        a.id === editingAgentId ? { ...a, expertise: editExpertise.trim() || undefined, profileNotes: editNotes.trim() || undefined } : a
      )
    )
  ...
  function handleToolConnected(agentId: string, toolId: string) {
    setConnectedToolsByAgent((prev) => {
      const next = { ...prev }
      if (!next[agentId]) next[agentId] = new Set()
      next[agentId] = new Set([...next[agentId], toolId])
      return next
    })
  }
      ...
      <div className="mx-auto max-w-4xl px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
        ...
        <div className="space-y-3">
          {agentList.map((agent) => {
            ...
            const connected = connectedToolsByAgent[agent.id] || new Set<string>()
