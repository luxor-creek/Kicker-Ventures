...
export function CompanyChat({ onMention }) {
  const [channels] = useState(chatChannels);
  ...
  const mentionSuggestions = mentionQuery !== null ? allMembers.filter(
    (m) => m.name.toLowerCase().startsWith(mentionQuery.toLowerCase())
  ) : [];
  useEffect(() => {
  ...
  function sendMessage() {
    ...
    const newMsg = {
      ...
      senderType: "admin",
      message: input.trim(),
      timestamp: (/* @__PURE__ */ new Date()).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" }),
      mentions: mentionedIds.length > 0 ? mentionedIds : void 0
    };
  ...
  function renderMessageText(text) {
    ...
    return parts.map((part, i) => {
      if (part.startsWith("@")) {
        return <span key={i} className="rounded bg-accent/15 px-1 font-semibold text-accent">
            {part}
          </span>;
      }
      if (part.startsWith("#")) {
        return <span key={i} className="rounded bg-primary/10 px-1 font-medium text-primary/80">
            {part}
          </span>;
      }
  ...
  return <div className="flex flex-1 overflow-hidden">
    ...
    className={cn(
      ...
      showChannelList ? "w-full sm:w-[220px]" : "hidden sm:flex sm:w-[220px]"
    )}
  >
    ...
    return <button
      ...
      className={cn(
        ...
        isActive ? "bg-accent/10 font-semibold text-accent" : "text-muted-foreground hover:bg-muted hover:text-foreground"
      )}
    >
                {ch.id === "clients" ? <Lock className="h-3.5 w-3.5 shrink-0" /> : <Hash className="h-3.5 w-3.5 shrink-0" />}
                <span className="truncate">{ch.name}</span>
                {unread > 0 && !isActive && <span className="ml-auto flex h-5 min-w-5 items-center justify-center rounded-full bg-accent px-1.5 text-[10px] font-bold text-accent-foreground">
                    {unread}
                  </span>}
              </button>;
  })}
  ...
  >
          ...
          {activeChannelData?.id === "clients" ? <Lock className="h-4 w-4 text-muted-foreground" /> : <Hash className="h-4 w-4 text-muted-foreground" />}
          <div className="min-w-0 flex-1">
            <h3 className="text-sm font-bold text-foreground">{activeChannelData?.name}</h3>
            {activeChannelData?.description && <p className="truncate text-[11px] text-muted-foreground">{activeChannelData.description}</p>}
          </div>
    ...
    /* Messages */
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-4">
          <div className="mx-auto max-w-3xl space-y-4">
            {channelMessages.length === 0 && <div className="py-12 text-center">
            ...
            {channelMessages.map((msg) => <div key={msg.id} className="group flex gap-3">
                <Avatar className="mt-0.5 h-8 w-8 shrink-0">
                  {msg.senderAvatar ? <AvatarImage src={msg.senderAvatar} alt={msg.senderName} /> : null}
    ...
    /* Mention autocomplete dropdown */
            ...
            {mentionQuery !== null && mentionSuggestions.length > 0 && <div className="absolute bottom-full left-0 right-0 mb-1 rounded-lg border border-border bg-card p-1 shadow-lg">
                {mentionSuggestions.map((member, i) => <button
    key={member.id}
  ...
  >
                    {member.type === "agent" ? <Bot className="h-3.5 w-3.5 text-accent" /> : member.type === "admin" ? <Shield className="h-3.5 w-3.5 text-accent" /> : <User className="h-3.5 w-3.5 text-muted-foreground" />}
    ...
    className={cn(
      ...
      isListening ? "bg-destructive text-destructive-foreground animate-pulse" : "border border-border bg-background text-muted-foreground hover:text-foreground"
    )}
